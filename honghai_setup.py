...
asset_options = gymapi.AssetOptions()
# asset_options.fix_base_link = True
usb_asset = gym.load_asset(sim, asset_root,  'urdf/usb/usb.urdf' , asset_options)

# create hole
asset_options = gymapi.AssetOptions()
# asset_options.fix_base_link = True
hole_asset = gym.load_asset(sim, asset_root,  'urdf/usb/factory_rectangular_hole_8mm.urdf' , asset_options)
...
for i in range(num_envs):
    ...
    # add hole
    hole_pose.p.x = table_pose.p.x + np.random.uniform(-0.2, 0.1)
    hole_pose.p.y = table_pose.p.y + np.random.uniform(-0.3, 0.3)
    ...
    # add region
    usb_pose.p.x = table_pose.p.x + np.random.uniform(-0.2, 0.1)
    usb_pose.p.y = table_pose.p.y + np.random.uniform(-0.3, 0.3)
    usb_pose.p.z = table_dims.z #+ 0.001
    usb_pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 0, 1), np.random.uniform(-math.pi, math.pi))
