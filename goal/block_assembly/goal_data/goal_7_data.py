...
class Goal_7(Goal):
    def __init__(self):
        ...
        goal_pos_1 = torch.Tensor(( 0,0.0145, 0.0145)).to(device=self.device)

        goal_rot_1 = torch.Tensor((-0.5*np.pi,np.pi/2,  0)).to(device=self.device)
        ...
        goal_pos_2 = torch.Tensor((0,-0.0145,  0.0145)).to(device=self.device)

        goal_rot_2 = torch.Tensor((-0.5*np.pi,np.pi/2,  0)).to(device=self.device)
        ...
        goal_pose_3.p = gymapi.Vec3(-0.005, 0, 0.0318)
